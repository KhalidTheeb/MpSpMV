...
short int * new_array(const size_t N, const memory_location loc) 
{ 
    //dispatch on location
    if (loc == HOST_MEMORY){
        return (short int*) malloc(N * sizeof(short int));
    }
    else{
        void * ptr = 0;
...
void delete_array(short int * p, const memory_location loc) 
{ 
    //dispatch on location
    if (loc == HOST_MEMORY){
        free(p);
...
short int * new_device_array(const size_t N) { return new_array(N, DEVICE_MEMORY); }
short int * new_host_array(const size_t N)   { return new_array(N, HOST_MEMORY); }
